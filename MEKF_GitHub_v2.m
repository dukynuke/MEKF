...
% figure ; sgtitle('3-\sigma Boundaries and Estimation Errors','FontWeight','bold') ;
% subplot(3,1,1) ; ha = plot(3*sqrt(diagsP(:,1)),'b') ; hold on ; plot(-3*sqrt(diagsP(:,1)),'b') ; hb = plot(deg2rad(r_true)-r_up,'r') ; grid on ;
% lgnd = legend([ha hb],'3\sigma Boundaries','Estimation Error','FontWeight','bold','location','best') ;
