...
IMU.Gyroscope = gyroparams( ...
    'MeasurementRange',Inf, ...
    'Resolution',0.00059875*0, ...
    'ConstantBias',1*deg2rad(0.04)*[1 1 1], ...
    'AxesMisalignment',0, ...
...
Q = [g_var/3 zeros(3);zeros(3) zeros(3)] ;  %
...
% Measurement Update
e = [R_TRIAD*[0;0;1];R_TRIAD*[27.555;-2.4169;-16.0849]/norm([27.555;-2.4169;-16.0849])] - ...
    [R_prop*[0;0;1];R_prop*[27.555;-2.4169;-16.0849]/norm([27.555;-2.4169;-16.0849])] ; 
R = [sig_a^2*eye(3) zeros(3);zeros(3) sig_m^2*eye(3)] ;   
H = [ssym(R_up*[0;0;1]) zeros(3);ssym(R_up*[27.555;-2.4169;-16.0849]/norm([27.555;-2.4169;-16.0849])) zeros(3)] ;
K = P*H'*inv(H*P*H'+R) ;
P = (eye(6)-K*H)*P ;
diagsP(i,:) = diag(P) ;
